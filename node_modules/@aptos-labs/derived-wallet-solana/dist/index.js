...
__export(index_exports, {
  ...
  SolanaDerivedAccount: () => SolanaDerivedAccount,
  SolanaDerivedPublicKey: () => SolanaDerivedPublicKey,
  SolanaDerivedWallet: () => SolanaDerivedWallet,
  createAccountAuthenticatorForSolanaTransaction: () => createAccountAuthenticatorForSolanaTransaction,
  createMessageForSolanaTransaction: () => createMessageForSolanaTransaction,
  createSignInMessage: () => import_wallet_standard_util4.createSignInMessage,
  createSiwsEnvelope: () => createSiwsEnvelope,
  createSiwsEnvelopeForAptosStructuredMessage: () => createSiwsEnvelopeForAptosStructuredMessage,
  createSiwsEnvelopeForAptosTransaction: () => createSiwsEnvelopeForAptosTransaction,
  createTransactionStatement: () => import_derived_wallet_base7.createTransactionStatement,
  defaultSolanaAuthenticationFunction: () => defaultSolanaAuthenticationFunction,
  extendSolanaWallet: () => extendSolanaWallet,
  getChainName: () => import_derived_wallet_base7.getChainName,
  getEntryFunctionName: () => import_derived_wallet_base7.getEntryFunctionName,
...
function createSiwsEnvelopeForAptosTransaction(input) {
  ...
  const statement = (0, import_derived_wallet_base.createTransactionStatement)(rawTransaction, domain);
...
var SolanaDerivedPublicKey = class _SolanaDerivedPublicKey extends import_ts_sdk2.AccountPublicKey {
  ...
  verifySignature({ message, signature }) {
    ...
    if (parsedSigningMessage.type === "structuredMessage") {
      messageBytes = (0, import_derived_wallet_base2.encodeStructuredMessage)(
      ...
      messageBytes = (0, import_wallet_standard_util.createSignInMessage)(siwsEnvelopeInput);
    }
    const ed25519PublicKey = new import_ts_sdk2.Ed25519PublicKey(
  ...
  static deserialize(deserializer) {
    ...
    return new _SolanaDerivedPublicKey({
      domain,
      solanaPublicKey,
      authenticationFunction
    });
...
var defaultSolanaAuthenticationFunction = "0x1::solana_derivable_account::authenticate";
...
async function signAptosMessageWithSolana(input) {
  ...
  return (0, import_derived_wallet_base4.mapUserResponse)(response, (output) => {
    const signature = new import_ts_sdk3.Ed25519Signature(output);
...
async function signAptosTransactionWithSolana(input) {
  ...
  if (solanaWallet.signIn) {
    const signInResponse = await wrapSolanaUserResponse(
      ...
      if (error instanceof import_wallet_adapter_base2.WalletError || !solanaWallet.signMessage)
        throw error;
      return void 0;
    });
    if (signInResponse) {
      return (0, import_derived_wallet_base5.mapUserResponse)(signInResponse, (output) => {
  ...
  if (solanaWallet.signMessage) {
    const response = await wrapSolanaUserResponse(
      solanaWallet.signMessage((0, import_wallet_standard_util2.createSignInMessage)(siwsInput))
    );
...
function createAccountAuthenticatorForSolanaTransaction(signatureBytes, solanaPublicKey, domain, authenticationFunction, signingMessageDigest) {
  const signature = new import_ts_sdk4.Ed25519Signature(signatureBytes);
  ...
  const abstractPublicKey = new import_derived_wallet_base5.DerivableAbstractPublicKey(
    solanaPublicKey.toBase58(),
    domain
  );
...
function createMessageForSolanaTransaction(input) {
  const { rawTransaction, authenticationFunction, solanaPublicKey, domain } = input;
  const signingMessage = (0, import_ts_sdk4.generateSigningMessageForTransaction)(rawTransaction);
  const message = import_ts_sdk4.AbstractedAccount.generateAccountAbstractionMessage(
    signingMessage,
    authenticationFunction
  );
  const signingMessageDigest = (0, import_ts_sdk4.hashValues)([message]);
...
// src/SolanaDerivedAccount.ts
var import_ts_sdk6 = require("@aptos-labs/ts-sdk");
var import_wallet_standard_util3 = require("@solana/wallet-standard-util");
var import_tweetnacl = __toESM(require("tweetnacl"));
var SolanaDerivedAccount = class extends import_ts_sdk6.AbstractedAccount {
  constructor(params) {
    const {
      solanaKeypair,
      domain,
    ...
    super({
      ...
      authenticationFunction,
      signer: (digest) => import_tweetnacl.default.sign.detached(digest, solanaKeypair.secretKey)
    });
  ...
  signTransactionWithAuthenticator(transaction) {
    ...
    const messageToSign = (0, import_wallet_standard_util3.createSignInMessage)(siwsInput);
    const signatureBytes = import_tweetnacl.default.sign.detached(
      messageToSign,
...
// src/extendSolanaWallet.ts
function extendSolanaWallet(solanaWallet, authenticationFunction = defaultSolanaAuthenticationFunction) {
  const extended = solanaWallet;
  extended.getAptosPublicKey = (solanaPublicKey) => new SolanaDerivedPublicKey({
    solanaPublicKey,
    domain: window.location.host,
    authenticationFunction
  });
  extended.signAptosTransaction = (rawTransaction) => signAptosTransactionWithSolana({
    solanaWallet,
    authenticationFunction,
    rawTransaction,
    domain: window.location.host
...
var import_wallet_adapter_base3 = require("@solana/wallet-adapter-base");
...
function setupAutomaticSolanaWalletDerivation(options = {}) {
  ...
  const deriveAndRegisterWallet = (wallet) => {
    ...
    registrations[wallet.name]?.();
    registrations[wallet.name] = api.register(derivedWallet);
  };
  for (const wallet of api.get()) {
    if ((0, import_wallet_adapter_base3.isWalletAdapterCompatibleStandardWallet)(wallet) && isWhitelisted(wallet)) {
  ...
  const offUnregister = api.on("unregister", (wallet) => {
    if ((0, import_wallet_adapter_base3.isWalletAdapterCompatibleStandardWallet)(wallet)) {
...
// src/index.ts
var import_wallet_standard_util4 = require("@solana/wallet-standard-util");
var import_derived_wallet_base7 = require("@aptos-labs/derived-wallet-base");
