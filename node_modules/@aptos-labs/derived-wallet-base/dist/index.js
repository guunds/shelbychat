...
function createStructuredMessageStatement({
  message,
  chainId
}) {
...
var transactionWithDataSigningMessagePrefix = (0, import_ts_sdk3.hashValues)([
  import_ts_sdk3.RAW_TRANSACTION_WITH_DATA_SALT
]);
...
function parseAptosSigningMessage(message) {
  ...
  if (parsedRawTransaction) {
    ...
    if (parsedRawTransaction instanceof import_ts_sdk3.RawTransaction) {
      ...
      rawTransaction = secondary_signer_addresses.length > 0 ? new import_ts_sdk3.MultiAgentTransaction(
        raw_txn,
        secondary_signer_addresses,
        fee_payer_address
      ) : new import_ts_sdk3.SimpleTransaction(raw_txn, fee_payer_address);
    } else {
